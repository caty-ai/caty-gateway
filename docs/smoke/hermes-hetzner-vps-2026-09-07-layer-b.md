...
`backend=hermes:http://127.0.0.1:18773` — `gen_first` 36.2 s (text-only turn before the TTS fix) and 51.6 s (with voice, `tts_first` +2.0 s). The whole latency is the smoke Hermes profile's 8B local model on CPU; during this session Ollama held three models in memory (load average ≈46), which coincided with turns ~10 s slower than in layer A.

## Notes

- The agent introduces itself with the persona of the non-member test profile the smoke profile was created from (persona file only; no keys or memory). The owner noted this persona had the best Japanese of the four routes.
