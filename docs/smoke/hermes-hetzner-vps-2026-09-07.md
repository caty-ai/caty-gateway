...
| 2 | doctor all PASS | PASS | `doctor --backend hermes --port 18772` with `CATY_HERMES_URL` / `CATY_HERMES_API_KEY` loaded from the env file: 14 PASS, 0 FAIL (`hermes API key`, `hermes models` both PASS; without the env: 12 PASS / 2 FAIL, as in the superseded record) |
