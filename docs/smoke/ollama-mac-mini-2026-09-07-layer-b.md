...
host: mac-mini (reached over the tailnet)
os: macOS 26.5.2 (arm64)
date: 2026-09-07 (owner's session, evening JST; host log times below are the host's local time)
layer: B
caty_gateway_version: caty-gateway 0.1.4 (the isolated layer A install of 2026-09-06)
result: PASS
---
...
`backend=openai-compat:huihui_ai/granite3.2-vision-abliterated:latest` — three turns in the session: `gen_first` 0.3 s / 0.4 s / 1.3 s; `tts_first` 2.3 s and 2.4 s on the two turns whose full log line was captured (`mode=legacy`, audio produced on all three).

## Notes

- The first reply's voice was the host's fallback synthesiser (the owner: "sounds like the PC default"): the smoke member had no `CATY_TTS_VOICE`, so the Fish proxy on the host refused (`Missing 'voice' field`) and the gateway fell back to the OpenClaw TTS capability. `CATY_TTS_VOICE` was set to the built-in preset `0089dce5fefb4c6ba9b9f2f0debe1ddc` (`voice_presets.py`) during the session and the service re-bootstrapped; later turns used that voice.
