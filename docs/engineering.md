...
2. Delete the four storage locations above
...
- `tools/scrub-audit.sh` detects personal names, internal paths, and strings that look like secrets. Add exceptions to `.scrub-allow` with a reason
- `tools/check_publication_gate.py` is vendored byte-identical from the source of truth for the pre-publication gate (family-dev-handbook). Locally, `make gate` runs the same checks
